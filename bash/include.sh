...
function bootYAML() {
	...
	bootPART="$(find_boot)";
	bootUUID="$(find_UUID ${bootPART})"
		std_o="${std_o}boot: EFI\n"
		std_o="${std_o}  uuid: ${bootUUID}\n"
		std_o="${std_o}  partition: ${bootPART}\n"
		std_o="${std_o}disks:\n"
	dpath="/"
		std_o="${std_o}  path: ${dpath}\n"
	dpool="${$(df ${dpath} | tail -n 1 | awk '{print $1}')%/*}"
		std_o="${std_o}  pool: ${dpool}\n"
	ddataset="${$(df ${dpath} | tail -n 1 | awk '{print $1}')#*/}"
		std_o="${std_o}  dataset: ${ddataset}\n"

		std_o="${std_o}cmdline:\n"
		# refind, grub, ...
		std_o="${std_o}bootloader:\n"
		std_o="${std_o}  properties:\n"
		# resolution
		std_o="${std_o}  entry:\n"
		# generated by reading all entries for loaders like grub or refind
		# entry:"Gentoo Linux 6.5.5-gentoo safe/g2"
		#   icon:
		#   loader:
		#   initrd:
		#   options:
		#     -UUID:17552437060435209316
		#     -real_root:ZFS=safe/g2
		#	  -dozfs
		#	  -default
		#     -rw
		#     -scandelay:2
		# entry:"Gentoo Linux 6.5.7-gentoo jupiter/plasma"
		# [entry] "[variant] Linux [VERSION] [location]"
		# 	-> fs type | location { UUID, [fs-type assertions], [fs args] }
		#   -> kernel | version + initrd
		#   -> options | { scandelay=2 rw default }
		# -entry:Jupiter.Plasma-Gentoo		// HOST.VARIANT
		#   location:jupiter/plasma
		#     UUID:17552437060435209316
		#     fstype:zfs
		#	  options:
		#       -dozfs
		#       -real_root=ZFS=jupiter/plasma

		#   kernel:
		#     version:6.5.7-gentoo/vmlinuz
		#     initrd:6.5.7-gentoo/jupiter.initramfs
		#	  compress:lz4
		#   
		#   options:
		#     -rw
...
function kernelLatest(){

	_latest="$(git ls-remote https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git | \grep -F -v 'rc' | tail -n 1)"
	_latest="${_latest##*/v}";
	_latest="${_latest%\^*}";
	echo "linux-$_latest-gentoo"
}
